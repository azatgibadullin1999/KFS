...
constexpr std::size_t g_page_directory_mapping_address = std::numeric_limits<std::size_t>::max() - ~(kfs::page::g_page_mask);
constexpr std::size_t g_page_tables_mapping_address = std::numeric_limits<std::size_t>::max() - (kfs::page::g_page_size * 1024 - 1);

AddressSpace::AddressSpace(page::Page::MemoryMapping page_directory)
: _page_directory{ page_directory } {
	auto&& directory = _get_page_directory().entrys();
...
void AddressSpace::load() noexcept {
	_page_directory.virtual_address = reinterpret_cast<page::Page *>(g_page_directory_mapping_address);
	asm volatile (
	"mov %0, %%cr3\n\t"
		:
...
std::span<page::Table> AddressSpace::_get_page_tables() const noexcept {
	return std::span<page::Table>{
		*reinterpret_cast<std::array<page::Table, 1023>*>(g_page_tables_mapping_address)
	};
}

page::Directory &AddressSpace::_get_page_directory() const noexcept {
	return *reinterpret_cast<page::Directory *>(_page_directory.virtual_address);
}

kfs::phys_addr_t AddressSpace::_get_page_directory_physical_address() const noexcept {
	return _page_directory.physical_address;
}

kfs::virt_addr_t AddressSpace::allocate() {
	auto&& directory = _get_page_directory().entrys();
	auto&& tables = _get_page_tables();
	...
	if (dit == std::ranges::end(enumirate)) {
		...
		directory[*dit].set(std::addressof(_get_page_tables()[*dit]))
	...
	table[*tit].set(reinterpret_cast<kfs::virt_addr_t>(*dit << 22 | *tit << 12))
		.phys_addr(PhysicalMemory::instance().allocate())
		.present(true);

	return reinterpret_cast<kfs::virt_addr_t>(*dit << 22 | *tit << 12);
}

void AddressSpace::deallocate(kfs::virt_addr_t memory) {
	std::size_t di = reinterpret_cast<std::size_t>(memory)       >> 22;
	std::size_t ti = reinterpret_cast<std::size_t>(memory) << 12 >> 22;

	auto &&entry = _get_page_tables()[di].entrys()[ti];
