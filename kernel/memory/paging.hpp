...
#ifndef KFS_KERNEL_SERVICE_MEMORY_PAIGING_HPP
# define KFS_KERNEL_SERVICE_MEMORY_PAIGING_HPP
...
struct Page {
	struct MemoryMapping {
		...
		kfs::phys_addr_t physical_address;
	};
...
struct Directory : private Page {
	struct Entry : private detail::DirectoryEntry {
		struct Setter {
			Setter(detail::DirectoryEntry &entry, kfs::virt_addr_t address)
			...
			Setter&& phys_addr(kfs::phys_addr_t address) && noexcept {
		...
		kfs::phys_addr_t get_phys_addr() const noexcept     { return base_high << 16 | base_low << 12; }
		Setter set(kfs::virt_addr_t address) { return Setter{*this, address}; }
	};
...
struct Table : private Page {
	struct Entry : private detail::TableEntry {
		struct Setter {
			Setter(detail::TableEntry &entry, kfs::virt_addr_t address)
			: _entry{ entry }, _address{ address } { }
			~Setter();
			Setter&& present(bool val) && noexcept              { _entry.present = val;             return std::move(*this); }
			Setter&& read_write(bool val) && noexcept           { _entry.read_write = val;          return std::move(*this); }
...
#endif // KFS_KERNEL_SERVICE_MEMORY_PAIGING_HPP
