...
/*   Updated: 2024/06/11 20:31:49 by larlena          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
public:
	...
	constexpr ~vector_impl() {
		for (size_type i = 0; i != mSize; i++)
			std::destroy_at(&mArray[i]);
		mAllocator.deallocate(mArray, mCapacity * sizeof(value_type));
	}

	vector_impl	&operator = (vector_impl &&other) noexcept(noexcept(std::declval<T>()(std::move(std::declval<T>())))) {
		std::swap(*this, other);
		return *this;
	} 
...
public :
	...
	vector_impl(count, alloc) {
		for (;mSize != count; ++mSize) {
			std::construct_at(&mArray[mSize], value);
		}
	}
	...
	vector_impl(other.mCapacity, other.mAllocator) {
		for (; mSize != other.mSize(); ++mSize)
			std::construct_at(&mArray[mSize], other[mSize]);
	}

	constexpr vector(vector &&other) noexcept :
	vector_impl(std::move(other)) { }
	...
	vector	&operator = (vector &&other) noexcept(noexcept(std::declval<T>()(std::move(std::declval<T>())))) {
		return vector_impl::operator = (std::move(other));
	}
	...
	private :
		...
		void	__hidden_insert(iterator pos, size_type count, const_reference value, true_type) {
			...
			difference_type	diff = std::distance(begin(), pos);
			auto	srcBegin = reverse_iterator(mArray + mSize - 1);
			auto	srcEnd = reverse_iterator(mArray + diff);
			auto	dst = reverse_iterator(mArray + count + mSize - 1);

			std::move(srcBegin, srcEnd, dst);
			for (iterator it = srcEnd, ite = srcBegin; it != ite; ++it) {
		...
		template <typename InputIt>
		void	__hidden_insert(iterator pos, InputIt first, InputIt last) {
			difference_type count = std::distance(first, last);
			difference_type	diff = std::distance(begin(), pos);
...
template <typename T>
bool	operator == (const std::pmr::vector<T> &lhs, const std::pmr::vector<T>&rhs) {
...
template <typename T>
bool	operator < (const std::pmr::vector<T> &lhs, const std::pmr::vector<T>&rhs) {
	return std::lexicographic_compare(lhs.begin(), lhs.end(), rhs.begin(), rhs.end());
}
...
template <typename T>
void	std::swap(std::pmr::vector<T> &lhs, std::pmr::vector<T> &rhs) {
	lhs.swap(rhs);
}
